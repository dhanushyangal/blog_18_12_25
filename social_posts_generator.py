...
def save_linkedin_post_to_supabase(blog_slug: str, linkedin_post_data: Dict[str, Any], 
    ...
    try:
        ...
        if existing.data:
            ...
            supabase.table('linkedin_posts').insert(post_data, returning="minimal").execute()
            return {"status": "success", "message": "LinkedIn post saved successfully", "id": None}
...
def save_instagram_post_to_supabase(blog_slug: str, instagram_post_data: Dict[str, Any],
    ...
    try:
        ...
        if existing.data:
            ...
            # Insert new post (use returning='minimal' so RLS on SELECT doesn't block success)
            post_data = {
                'blog_post_id': blog_post_id,
                'blog_slug': blog_slug,
                'caption': instagram_post_data.get('caption', ''),
            ...
            supabase.table('instagram_posts').insert(post_data, returning="minimal").execute()
            # Minimal returning = no row data; success means no exception raised
            return {"status": "success", "message": "Instagram post saved successfully", "id": None}
